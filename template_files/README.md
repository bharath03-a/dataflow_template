...
python -m dataflow_starter_kit.pipeline --input=5 --runner=DirectRunner
```
...
docker run --rm --entrypoint /bin/bash dataflow-template:latest \
  -c "python -m dataflow_starter_kit.pipeline --input=1 --runner=DirectRunner"
...
```bash
python -m dataflow_starter_kit.pipeline \
  --input=1 \
